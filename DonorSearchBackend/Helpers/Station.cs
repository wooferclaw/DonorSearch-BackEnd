...
namespace DonorSearchBackend.Helpers
{
    ...
    public class BloodStation
    {
        ...
        public static async Task<string> GetBloodStationsByVkIdTask(int vkId)
        {
            ...
            var currentUser = new DAL.User();
            currentUser = UserRepository.GetUserByVkId(vkId);
            ...
            using (var graphQlClient = new GraphQLClient(apiPath + apiKey))
            {
                ...
                foreach (var station in stations)
                {
                    ...
                    station.requrement_of_user_blood = currentUser.CheckForBloodNeccesity(station.need_requests);

                    //check if station accepts first timers
                    station.accept_first_timers = DonationRepository.GetDonationByVkId(currentUser.vk_id).All(c => cityId != currentUser.city_id);

                    //check if station accepts users without registration
                    if (currentUser.has_registration.HasValue && !currentUser.has_registration.Value && station.without_registration.Value)
